...
const regex = /\{\/\* Online Users \*\/\}.*?\{\/\* Messages \*\/\}/s;
const replacement = `
                    ...
                    <div className="flex gap-2 mb-4 bg-slate-900/50 p-1.5 rounded-2xl border border-slate-800" dir="rtl">
                      ...
                      >
                        ...
                        طلبات المراسلة
                        {conversations.filter(c => !c.isGroup && profile?.friends && !profile.friends.includes(c.uid) && c.uid !== 'global' && c.unread).length > 0 && (
                    ...
                    {chatView === 'requests' ? (
                      <div className="space-y-2">
                        {conversations.filter(c => !c.isGroup && profile?.friends && !profile.friends.includes(c.uid) && c.uid !== 'global').length === 0 ? (
                          ...
                          conversations.filter(c => !c.isGroup && profile?.friends && !profile.friends.includes(c.uid) && c.uid !== 'global').map(conv => {
...
if (code.includes('{/* Online Users */}')) {
  // First, find the exact start and end to replace
  const startIdx = code.indexOf('{/* Online Users */}');
  const endIdx = code.indexOf('{/* Messages */}');
  if (startIdx !== -1 && endIdx !== -1) {
    const newCode = code.substring(0, startIdx) + replacement + code.substring(endIdx + 16);
    fs.writeFileSync('src/components/ChatBubble.tsx', newCode);
    console.log("Success");
  } else {
